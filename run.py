...
server = Popen([PYTHON, pathjoin(PROJECT_ROOT, "src", "backend", "server.py"), "debug"])
browser = threading.Thread(target=lambda: webbrowser.open('http://127.0.0.1:8080')) #the URL here is hardcoded and needs to match what server.py spins up on

browser.start()  #open a browser to the webapp's page (nonblocking)
server.wait()    #join the server thread              (blocking)
